...
public class ClassroomMemberServiceImpl extends ServiceImpl<ClassroomMemberMapper, ClassroomMember> implements ClassroomMemberService {
    ...
    @Override
    public void removeClassroomMember(Long classroomMemberId) {
        SysUser currentTeacher = sysUserService.getCurrentUser();

        if (currentTeacher.getUserType() != UserTypeEnum.TEACHER) {
            throw new BusinessException(ResultCodeEnum.FORBIDDEN, "只有老师可以移除课堂成员");
        }
        ...
        if (classroom.getStatus() != ClassroomStatusEnum.NORMAL) {
            throw new BusinessException(ResultCodeEnum.ILLEGAL_STATE, "已归档课堂不能移除成员");
        }
        ...
        boolean removed = lambdaUpdate()
                .eq(ClassroomMember::getId, classroomMemberId)
                .eq(ClassroomMember::getDeleted, DeletedStatusEnum.NORMAL)
                .set(ClassroomMember::getDeleted, DeletedStatusEnum.DELETED)
                .set(ClassroomMember::getUpdatedById, currentTeacher.getId())
                .set(ClassroomMember::getUpdateTime, now)
                .update();
    ...
    @Override
    public ClassroomMemberDetailVO getClassroomMemberDetail(Long classroomMemberId) {
        SysUser currentUser = sysUserService.getCurrentUser();
        ...
        if (currentUser.getUserType() == UserTypeEnum.TEACHER) {
            if (!classroom.getTeacherId().equals(currentUser.getId())) {
                throw new BusinessException(ResultCodeEnum.FORBIDDEN, "无权查看该课堂成员");
            }
        } else if (currentUser.getUserType() == UserTypeEnum.STUDENT) {
            long myMemberCount = lambdaQuery()
            ...
            if (myMemberCount == 0) {
                throw new BusinessException(ResultCodeEnum.FORBIDDEN, "加入课堂后才能查看课堂成员");
            }
        } else {
            throw new BusinessException(ResultCodeEnum.FORBIDDEN, "无权查看课堂成员");
        }
    ...
    @Override
    public Page<ClassroomMemberListVO> pageClassroomMembers(Long classroomId,
        ...
        LambdaQueryWrapper<ClassroomMember> wrapper = Wrappers.lambdaQuery();
        wrapper.eq(ClassroomMember::getClassroomId, classroomId);
        wrapper.eq(ClassroomMember::getDeleted, DeletedStatusEnum.NORMAL);

        ClassroomMemberPageQueryDTO.Filter filter = dto.filter();

        if (filter != null && filter.keyword() != null && !filter.keyword().isBlank()) {
            wrapper.like(ClassroomMember::getStudentName, filter.keyword().trim());
        }

        wrapper.orderByDesc(ClassroomMember::getJoinedTime);

        Page<ClassroomMember> pageResult = page(new Page<>(dto.page(), dto.size()), wrapper);
        List<ClassroomMemberListVO> records = pageResult.getRecords()
                .stream()
                .map(member -> new ClassroomMemberListVO(
                        member.getId(),
                        member.getStudentId(),
                        member.getStudentName(),
                        member.getJoinType(),
        ...
        Page<ClassroomMemberListVO> result = new Page<>();
