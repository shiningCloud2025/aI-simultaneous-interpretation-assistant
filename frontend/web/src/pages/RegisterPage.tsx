...
export function RegisterPage() {
  ...
  const doRegister = () => { showToast('注册成功！'); setTimeout(() => nav('/login'), 1000); };
  const [smsCount, setSmsCount] = useState(0);

  return (
    <AuthBox title="创建账号" subtitle="开始你的同声转译之旅">
      <div style={{ display: 'flex', marginBottom: 24, background: '#f5f3f0', borderRadius: 10, padding: 4 }}>
        <button onClick={() => setTab('email')} style={tb(tab === 'email')}>邮箱注册</button>
        <button onClick={() => setTab('phone')} style={tb(tab === 'phone')}>手机号注册</button>
      </div>
      {tab === 'email' ? (
        <>
          <Fg label="邮箱"><input placeholder="请输入邮箱" style={inp} /></Fg>
          <Fg label="密码"><input type="password" placeholder="至少6位密码" style={inp} /></Fg>
          <Fg label="确认密码"><input type="password" placeholder="再次输入密码" style={inp} /></Fg>
          <div style={{ marginBottom: 20, fontSize: 13, color: '#888' }}><input type="checkbox" style={{ accentColor: '#2c2c2c' }} /> 同意用户协议和隐私政策</div>
          <Btn onClick={doRegister}>注 册</Btn>
        </>
      ) : (
        <>
          <Fg label="手机号"><input placeholder="请输入手机号" style={inp} /></Fg>
          ...
          <Fg label="设置密码"><input type="password" placeholder="至少6位密码" style={inp} /></Fg>
          <Btn onClick={doRegister}>注 册</Btn>
        </>
      )}
