...
const log = { warn: (...args: any[]) => console.warn('[ExportPPTX]', ...args), error: (...args: any[]) => console.error('[ExportPPTX]', ...args) };
...
/**
 * Build PPTX blob from slides — adapted from OpenMAIC
 */
    ...
    for (const el of slide.elements) {
      ...
      // ── CHART ──
        ...
        if (el.themeColors.length === 10) {
          ...
          chartColors = (tinycolor(el.themeColors[0]) as any)
            .analogous(10)
            .map((c: any) => formatColor(c.toHexString()).color);
        } else {
          const len = el.themeColors.length;
          const supplement = (tinycolor(el.themeColors[len - 1]) as any)
            .analogous(10 + 1 - len)
            .map((c: any) => c.toHexString());
          chartColors = [...el.themeColors.slice(0, len - 1), ...supplement].map(
            (c: any) => formatColor(c).color,
          );
        }
